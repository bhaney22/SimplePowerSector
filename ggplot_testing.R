...
.data <- DF.results.f %>% 
  ...
  select(Fin.1.Mkt.share, Fin.1.price, Eta.1, alpha.phys, alpha.curr, GDP.curr, PRR.phys, PRR.curr) %>% 
  mutate(
    Fin.1.Mkt.share = Fin.1.Mkt.share %>% as.numeric, 
    Fin.1.price = Fin.1.price %>% as.numeric,
    Eta.1 = Eta.1 %>% as.numeric, 
    ...
    Eta.1_factor = case_when(
      .data$Eta.1 < 0.5 ~ "Low (1/3)",
      .data$Eta.1 >= 0.5 ~ "High (2/3)",
      TRUE ~ NA_character_
    ),
    Eta.1_factor = factor(Eta.1_factor, levels = c("Low (1/3)", "High (2/3)"))
  ) %>% 
  gather(key = "y.var", value = "y.val", alpha.phys, alpha.curr, GDP.curr, PRR.phys, PRR.curr)
...
.data %>% 
  ...
  facet_grid(y.var ~ Eta.1_factor, scales = "free_y") +
